...
write_csv(total_best_ball, file = csv_file_path, row.names = FALSE)


total_best_ball <- read_csv("total_best_ball.csv")

fantasy_model <- lm(HalfPPR ~ RB1_Stack + RB2_Stack +
                      WR1_Stack + WR2_Stack + TE_Stack,
                    data = total_best_ball)

bptest(fantasy_model)
dwtest(fantasy_model)

summary(fantasy_model)

vif(fantasy_model)

vcov_hac <- NeweyWest(fantasy_model, lag = 1, prewhite = FALSE, adjust = TRUE)
robust_se <- sqrt(diag(vcov_hac))

#---------------------------
# 5. Stargazer HTML Export (compact layout)
#---------------------------
stargazer(
  fantasy_model,
  type = "html",
  se = list(robust_se),
  title = "HAC Corrected Regression",
  out = "reg_table.html",
  column.sep.width = "4pt",
  single.row = TRUE
)

browseURL("reg_table.html")
